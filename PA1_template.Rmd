...
```{r Question 1: create a new dataset }
...
```{r Question 1 Part 1: Calculate total steps}
...
```{r Question 1 Part 2: Make histogram }
...
```{r Question 1 Part 3: Calculate mean and median}
...
The __mean__ of total steps taken per day is **`r mean_steps`**

The __median__ of total steps taken per day is **`r median_steps`**
...
```{r Question 2: create a new dataset }
...
``` {r Question 2 Part 1: Daily activity pattern}
...
```{r Question 2 Part2: maximum # of steps }
...
The five minute __interval__ starting at **`r max_interval`** contains **`r max_numberofstep`**  steps, which is the __maximum__.
...
```{r Question 3: create a new dataset }
...
```{r Question 3 Part 1: total# of missing value}
...
Total number of rows with missing data (NAs) is **`r missing_rows`**
...
```{r Question 3 Part 2: Fill in all of the missing value}
...
```{r Question 3 Part 3: new dataset without missing value}
...
- Number of rows in Original Dataset = **`r nrow_originaldataset`**  

- Number of rows in New Dataset = **`r nrow_newdataset`**  

- Number of rows with missing values (NAs) in the original data **`r nmissingrow_originaldataset`**  

- Number of rows with missing values (NAs) in the new data **`r nmissingrow_newdataset`**  
...
```{r Question 3 Part 4: Make a histogram}
...
- The mean of total steps taken per day for new dataset is **`r new_mean_steps`**  

- The median of total steps taken per day for new dataset is **`r new_median_steps`**  
...
Original    |  **`r mean_steps`** | **`r median_steps`**  

New         | **`r new_mean_steps`** | **`r new_median_steps`**  
________________________________________________________________________________
...
```{r Question 4: create a new dataset }
...
```{r Question 4 Part 1: find out given day is weekday or weekend}
...
```{r Question 4 Part 2: Make a panel plot }
